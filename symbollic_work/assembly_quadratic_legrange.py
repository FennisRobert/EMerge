...
def vf(_A,_B) -> Addition:
    return (_A*(2*A - 1)).G


def ef(_A,_B) -> Addition:
    return (4*_A*_B).G
...
replace = r'volume_coeff(V,\1,\2,0,0)*dot('

pattern2 = r'([A-Z])\*([A-Z])\*([A-Z])\*([A-Z])\*dot\('
replace2 = r'volume_coeff(V,\1,\2,\3,\4)*dot('
...
for i1, f1, i2, f2 in groups:
    output = f'            Dmat[ei{i1},ej{i2}] += CEE*({f1(A,B).dot(f2(C,D))})'
    print(re.sub(pattern, replace, output))
    #print(output)
    # output = f'            Fmat[ei{i1},ej{i2}] += Li*Lj*CFEE*({f1(A,B).dot(f2(C,D))})'
