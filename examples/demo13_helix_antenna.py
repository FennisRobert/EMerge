...
h_curve = em.geo.Curve.helix_lh(
    (0, 0, porth + dfeed / 2), (0, 0, porth + dfeed / 2 + L), rad0, 13, r_end=0.8 * rad0
)
...
block = (
    em.geo.Box(dfeed, dfeed, dfeed, position=h_curve.p0, alignment=em.CENTER)
...
abc_sel = airbox.boundary(
    exclude=("bottom",)
)  # absorbing boundary applied at bottom face of airbox
port_sel = feed.boundary(
    exclude=("front", "back")
)  # port faces on the feed (two opposite faces)
