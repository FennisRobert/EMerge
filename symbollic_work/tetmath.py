...
for (i,j),ei in zip(pairs, [e1, e2, e3, e4, e5, e6]):
    L1 = (avs[i] + bvs[i]*x + cvs[i]*y + dvs[i]*z)/(6*V)
    L2 = (avs[j] + bvs[j]*x + cvs[j]*y + dvs[j]*z)/(6*V)
    Wij = (L1 * gradient(L2) - L2 * gradient(L1))
    length = sp.sqrt((xs[j]-xs[i])**2 + (ys[j]-ys[i])**2 + (zs[j]-zs[i])**2)
    #print(sp.simplify(sp.expand(Wij)))
    if iii == 0:
        Evec = ei*Wij*length
    else:
        Evec = Evec + ei*Wij*length
    iii += 1
    
print(f'ex = {str(sp.simplify(Evec)[0]).replace("sqrt","np.sqrt")}')
print(f'ey = {str(sp.simplify(Evec)[1]).replace("sqrt","np.sqrt")}')
print(f'ez = {str(sp.simplify(Evec)[2]).replace("sqrt","np.sqrt")}')