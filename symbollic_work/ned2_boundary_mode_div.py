...
def de1f(_A,_B,_C) -> Addition:
    return _A.G*e1f(_A,_B,_C)

def de2f(_A,_B,_C) -> Addition:
    return _B.G*e2f(_A,_B,_C)
...
def df1f(_A,_B,_C) -> Addition:
    return _B.G*f1f(_A,_B,_C)

def df2f(_A,_B,_C) -> Addition:
    return _C.G*f2f(_A,_B,_C)
...
for i1, f1, i2, f2 in groups:
    output = f'            Dmat[ei{i1},ej{i2}] += Li*Lj*CEE*({f1(A,B,E).dot(f2(C,D,F))})'
    print(output)
