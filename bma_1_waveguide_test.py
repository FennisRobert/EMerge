...
with fem.Simulation3D('BMATest', 'DEBUG') as m:
    ...
    for i in range(port1.nmodes):
        ...
        d = MPLDisplay(m.mesh)
        d.add_object(fem.FaceSelection(m.physics.boundary_conditions[0].tags), opacity=0.8, show_edges=True, edge_opacity=1)
        d.add_portmode(port1, data.item(i).k0, 5, dv=(0,0.1*mm,0), XYZ = (X,Y,Z))
        d.show()

        d = MPLDisplay(m.mesh)
        d.add_object(fem.FaceSelection(m.physics.boundary_conditions[0].tags), opacity=0.8, show_edges=True, edge_opacity=1)
        d.add_portmode(port1, data.item(i).k0, 5, dv=(0,0.1*mm,0), XYZ = (X,Y,Z), field='H')
        d.show()
