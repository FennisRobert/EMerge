...
model.check_version("2.8.5")  # Checks version compatibility.
...
stubs = model.modeler.cylinder(
    W / 2,
    model.modeler.series(C1, lr1, lr2, lr1, C1),
    position=(model.modeler.series(x1, x2, x3, x4, x5), 0, 0),
    NPoly=10,
)
...
feed1out = em.geo.Cylinder(
    rout,
    lfeed,
    em.CoordinateSystem(em.ZAX, em.YAX, em.XAX, np.array([-lfeed, 0, h])),
    Nsections=12,
)
feed1in = em.geo.Cylinder(
    rin,
    lfeed + wi + W / 2,
    em.CoordinateSystem(em.ZAX, em.YAX, em.XAX, np.array([-lfeed, 0, h])),
    Nsections=8,
)
feed2out = em.geo.Cylinder(
    rout,
    lfeed,
    em.CoordinateSystem(em.ZAX, em.YAX, em.XAX, np.array([Lbox, 0, h])),
    Nsections=12,
)
feed2in = em.geo.Cylinder(
    rin,
    lfeed + wi + W / 2,
    em.CoordinateSystem(em.ZAX, em.YAX, em.XAX, np.array([Lbox - wi - W / 2, 0, h])),
    Nsections=8,
)
...
port2 = model.mw.bc.ModalPort(
    model.select.face.near(Lbox + lfeed, 0, h), 2, modetype="TEM"
)
